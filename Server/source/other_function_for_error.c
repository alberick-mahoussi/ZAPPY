...
int manag_teams_name(int i, char **argv, server_t * server)
{
    ...
    if (y == 0) {
        ...
        for (j = i + 1; argv[j] != NULL && argv[j][0] != '-'; j++, y++) {
            ...
            server->teams_players[y]->nb_players = 0;
            server->teams_players[y]->name_teams = strdup(argv[j]);
            server->teams_players[y]->id_chief = -1;
            server->teams_players[y]->chief_pos_x = -1;
            server->teams_players[y]->chief_pos_y = -1;
        }
        server->teams_players[y] = NULL;
    }
    return (j - 2);
}
...
int check_good_value_server(server_t *server)
{
    if (server->freq == 0)
        server->freq = 100;
    if (server->port == 0 || server->nb_players == 0)
